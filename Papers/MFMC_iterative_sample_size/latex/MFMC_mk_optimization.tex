...
an upper bound $M_k$ for all possible differences $m_k - m_{k-1}$, $k=2\ldots,K$.
For example, $M_k = p/ C_k - 1$ is such an upper bound because  $p/ C_k-1 \ge m_k-1 \ge m_k - m_{k-1}$, $k=2\ldots,K$.
...
\begin{subequations}\label{eq:Optimization_sample_size_Nz}
    ...
    \min \quad &\sigma_1^2  \sum_{k=1}^K \frac{ \rho_{1,k}^2 - \rho_{1,k+1}^2}{m_k},   \\
       \text{s.t.}\quad &  C_1 m_1 + \sum_{k=2}^K z_k C_k m_k \le b,       \label{eq:Optimization_sample_size_Nz_budget}  \\
                                & m_1\ge 1,\quad  m_k \ge m_{k-1}, \quad k=2\ldots,K,\\
                                & m_k - m_{k-1} \ge z_k, \quad M_k z_k \ge m_k - m_{k-1},  \quad k=2\ldots,K,  \label{eq:Optimization_sample_size_Nz_z}  \\
