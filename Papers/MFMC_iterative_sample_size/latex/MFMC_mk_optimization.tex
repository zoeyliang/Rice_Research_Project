...
an upper bound $\tilde{M}_k$ for all possible differences $m_k - m_{k-1}$, $k=2\ldots,K$.
If  $M_k$ is an upper bound for all feasible sample sizes  $m_k$, $k=2\ldots,K$,
then $\tilde{M}_k \le M_k-1$. We will use $\tilde{M}_k = M_k-1$ as an upper bound  
for all possible differences $m_k - m_{k-1}$, $k=2\ldots,K$.
Because $p/ C_k \ge m_k$, $k=2\ldots,K$, $M_k = p/ C_k$ is an upper bound for all feasible sample sizes  $m_k$.
...
\begin{subequations}\label{eq:Optimization_sample_size_Nz}
    ...
    \min \quad &\sigma_1^2  \sum_{k=1}^K \frac{ \rho_{1,k}^2 - \rho_{1,k+1}^2}{m_k},   \\
       \text{s.t.}\quad &  C_1 m_1 + \sum_{k=2}^K z_k C_k m_k \le b,       \label{eq:Optimization_sample_size_Nz_budget}  \\
                                & m_1\ge 1,\quad  m_k \ge m_{k-1}, \quad k=2\ldots,K,\\
                                & m_k - m_{k-1} \ge z_k, \quad (M_k-1) z_k \ge m_k - m_{k-1},  \quad k=2\ldots,K,  \label{eq:Optimization_sample_size_Nz_z}  \\
...
\eqref{eq:Optimization_sample_size_N}, namely that if  $m_k = m_{k-1}$, the $k$-th model does not
provide variance reduction, but its cost is included. Moreover, $m_1\ge 0$ s imposed instead of 
$m_1\ge 1$ to allow analytical solution of \eqref{eq:Optimization_sample_size_m_relaxed}  under
suitable conditions.
...
Integer sample sizes are computed by rounding.
For $x \in \real$, we use $\lfloor x \rfloor$ to denote the largest integer less than or equal to $x$,
...
$\mathbb{E}[A^{\text{MF}}] \not=  \mathbb{E}[u_1]$.
Rounding down may also lead to $\lfloor m_k^* \rfloor = \lfloor m_{k-1}^* \rfloor$ for some $k \in \{ 2, \ldots, K\}$, 
which means the $k$-th model does not contribute to variance reduction, but its cost is included in the
computational budget.
The case $\lfloor m_1^* \rfloor = 0$ can happen if the computational budget $b$ is small.
