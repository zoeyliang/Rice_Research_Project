...
For a $u \in  L_{\mathbb{P}}^2(W, {\mathcal U})$ we define
\begin{equation}   \label{eq:variance_of_u}
           \mathbb{E}\left[u \right]=\int_W u(\omega) \, \textup{d} \mathbb{P}(\omega) \in  {\mathcal U}
           \quad \mbox{ and } \quad
           \mathbb{V}[u] = \mathbb{E}\Big[  \big\| u - \mathbb{E}[u]  \big\|_{{\mathcal U}}^2 \Big] \in \real.
  \end{equation}




The goal is to estimate the expected value $\mathbb{E}[u_1 ]$ of a high fidelity model (HFM) $u_1 \in   L_{\mathbb{P}}^2(W, {\mathcal U})$.
...
and has variance $\mathbb{V}[A^{\text{MC}}_{N}] = N^{-1} \mathbb{V}[u_1 ]$.



The MFMC framework uses an ensemble of models with varying computational cost and fidelity 
to construct an estimator of $\mathbb{E}[u_1 ]$ with reduced variance, compared to the MC estimator.
In addition to the HFM $u_1 \in   L_{\mathbb{P}}^2(W, {\mathcal U})$ assume that we have lower-fidelity models (LFMs)
$u_2, \ldots u_K \in   L_{\mathbb{P}}^2(W, {\mathcal U})$ that can be sampled with lower computational cost.
The goal of MFMC is to allocate a fixed computational budget across these models to minimize estimator variance while maintaining unbiasedness.

Given $1 \le m_1 \le m_2 \le \ldots \le m_K$ and i.i.d.\ samples $\omega^{(1)}, \ldots, \omega^{(m_K)}$,
the MFMC estimator $A^{\text{MF}}$ combines an MC estimate of the HFM with differences 
of MC estimates of the LFMs.
...
\begin{equation}\label{eq:MFMC_estimator_independent}
   ...
   =: U_1 + \sum_{k=2}^K \alpha_k U_k,
\end{equation}
where
\begin{equation} \label{eq:MFMC_Yk}
       U_1 := A^{\text{MC}}_{1,m_1},\qquad 
       U_k := A^{\text{MC}}_{k,m_k} - A^{\text{MC}}_{k,m_{k-1}}
               =\left(1-\frac{m_{k-1}}{m_k}\right)
                 \left(A_{k,m_k\backslash m_{k-1}}^{\text{MC}}-A_{k,m_{k-1}}^{\text{MC}}\right), \quad k=2\ldots, K.
\end{equation}
In the formulation \eqref{eq:MFMC_estimator_independent}, 
the two terms in each correction are evaluated on independent sample sets, 
which simplifies variance analysis. 
%
Because $\mathbb{E}\big[ A^{\text{MC}}_{k,m} \big] = \mathbb{E}\big[ u_k\big]$, $m \in \nat$, $k\ge 1$,
it holds that  $\mathbb{E}[U_1] =  \mathbb{E}[u_1]$ and  
$\mathbb{E}[U_k] = 0$ for $k\ge 2$.
...
is biased.
To analyze the variance, we define the following quantities.
...
$\text{Cov}[u_k, u_j] := \mathbb{E}\big[\langle u_k - \mathbb{E}[u_k], u_j - \mathbb{E}[u_j \rangle_{{\mathcal U}} \big]$.
By definition, $\rho_{k,k}=1$. 
The variances of the components are
\begin{equation}\label{eq:Var_Yk}
    \mathbb{V}[U_1] = \frac{\sigma_1^2}{m_1}, \qquad 
    \mathbb{V}[U_k] = \left(\frac{1}{m_{k-1}} - \frac{1}{m_k}\right)\sigma_k^2, \;\; k=2\ldots, K.
\end{equation}
The correlations satisfy
\begin{equation}\label{eq:Cov_Yk}
\operatorname{Cov} [U_k,U_j ]=0, \quad  2\le k<j\le K, 
\quad \mbox{ and } \quad
\operatorname{Cov}[U_1,U_k] = -\!\left(\frac{1}{m_{k-1}} - \frac{1}{m_k}\right)\rho_{1,k}\sigma_1\sigma_k,
\quad  2\le k \le K, 
\end{equation}
...
If $m_k = m_{k-1}$ for some $k \in \{ 2, \ldots, K \}$, then the $k$-th term in  \eqref{eq:MFMC_variance0} 
vanishes and all $\alpha_k^* \in \real$ and also solutions.
...
\begin{equation}  \label{eq:MFMC_variance_ka}
    \mathcal{V}^{\text{MF}}(m_1, \ldots, m_K)
    :=  \mathbb{V}[A^{\text{MF}}]
    = \frac{\sigma_1^2}{m_1} 
...
Similarly,  \eqref{eq:MFMC_variance_b} shows that if $\rho_{1,k}^2 = \rho_{1,k+1}^2$ for some model $k$, 
then this model does not contribute to the variance reduction, i.e.,
\begin{equation}  \label{eq:MFMC_variance_kb}
    \mathcal{V}^{\text{MF}}(m_1, \ldots, m_K)
    :=  \mathbb{V}[A^{\text{MF}}]
   = \sigma_1^2\sum_{\rho_{1,k}^2 \not= \rho_{1,k+1}^2} \frac{ \rho_{1,k}^2 - \rho_{1,k+1}^2}{m_k}.
\end{equation}
The next step is to compute the sample sizes.
