...
\[
\sum_{k=1}^K C_k \left\lfloor N_k^* \right\rfloor\;\le\; \sum_{k=1}^K C_k\left\lfloor M_k^* \right\rfloor \;\le\; b.
\]
\end{theorem}
...
\For{$k = 2$ \KwTo $K$}{
    Update residual budget: $R_k = b - \sum_{j=1}^{k-1} C_j \left\lfloor M_j^* \right\rfloor$.\;
