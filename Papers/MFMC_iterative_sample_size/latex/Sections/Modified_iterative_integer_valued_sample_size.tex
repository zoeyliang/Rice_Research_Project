...
When the computational budget $b$ is small, the iterative scheme in \eqref{eq:Modified_sample_size_iterative_integer_proxy} may assign identical integer samples to consecutive fidelity levels. Since equal allocations do not contribute to variance reduction but still incur a cost, such configurations are inefficient. We therefore introduce a post-processing step that removes redundant levels and reallocates the budget across a reduced hierarchy.

The mechanism relies on the observation that if two consecutive levels satisfy $\lfloor M_{k-1}^* \rfloor = \lfloor M_k^* \rfloor$, then one level can be discarded without worsening the variance decay. Eliminating the less efficient of the two levels concentrates the fixed budget on fewer fidelities, thereby increasing the remaining sample sizes and improving the resulting estimator. This modification is particularly beneficial when $b$ is small relative to the size of the hierarchy.

To determine which level to remove, we adopt an efficiency-based comparison. Let $f_{\mathcal{M}}^{\text{Act}}=\sum_{i\in \mathcal{M}}\Delta_i/\overline{m}_i$ denote the actual variance for a given active set $\mathcal M$, where $\overline m_i=\lfloor M_i^* \rfloor$ denotes the integer allocation (via floor). For each candidate removal $j\in \{k-1,k\}$, we recompute the integer allocations under the same budget $b$ and evaluate $f_{\mathcal M\setminus \{j\}}^{\mathrm{Act}}$. The level yielding smaller variance after removal is discarded, ensuring that the retained hierarchy minimizes variance inflation. After eliminating a level, the reduced set must still satisfy the feasibility assumptions of MFMC in Theorem~\ref{thm:Sample_size_real}. The correlation monotonicity condition remains valid automatically when removing an intermediate level. The only nontrivial condition that must be verified is the cost–correlation ratio inequality, $\frac{\rho_{1,k-1}^2 - \rho_{1,k+1}^2}{C_{k-1}}<\frac{\rho_{1,k+1}^2 - \rho_{1,k+2}^2}{C_{k+1}}$, which guarantees that the resulting allocation produces a strictly increasing sequence of sample sizes and that each retained fidelity level contributes effectively to variance reduction. The complete algorithm -- iterative allocation followed by post-process elimination -- is summarized in Algorithm~\ref{algo:Iterative_MFMC_Algo}.
