...
When the computational budget $p$ is limited, the iterative allocation scheme in \eqref{eq:Modified_sample_size_iterative_integer_proxy} may yield identical integer-valued sample sizes across consecutive fidelity levels. Such allocations are inefficient: identical sample sizes provide no additional variance reduction while still incurring computational costs. To address this inefficiency, we introduce a post-processing procedure that dynamically adjusts the active model set during iteration, eliminating levels that would receive identical allocations.

The key insight is that when two consecutive levels $k-1$ and $k$ receive the same integer sample size $m_{k-1} = m_k$, one of these models can be removed without compromising variance reduction. The decision to remove model $k-1$ or $k$ is based on efficiency metrics. We remove the model whose exclusion minimizes the impact on the theoretical variance $\mathcal{V}^{\text{MF}}$ in \eqref{eq:MFMC_variance_optimal}, thereby preserving estimator efficiency. Formally, we compute the variance difference
\[
\Delta f_j = f_{\mathcal{M}}^{\text{Act}} - f_{\mathcal{M}\backslash\{j\}}^{\text{Act}}
\]
for $j = k-1, k$ and $\mathcal{M}$ is the selected model set and remove the model with larger $\Delta f_j$. Let $\overline{m_i}$ be the integer sample size of set $\mathcal{M}$  obtained using either floor or ceil. then $f_{\mathcal{M}}^{\text{Act}}=\sum_{i=1}^K \Delta_i/\overline{m_i}$; $f_{\mathcal{M}\backslash\{j\}}^{\text{Act}}$ can be obtained by deleting $j$-th model, and obtain the corresponding integer valued sample size with budget $b$ using the modified iterative scheme. Note that the reduced model set must still satisfy the theoretical conditions required for optimal MFMC estimation. Specifically, when considering the removal of a model at level $k$, the correlation monotonicity condition is automatically preserved in the reduced set. The critical verification is for the cost-correlation ratio condition $(\rho_{1,k-1}^2 - \rho_{1,k+1}^2)/C_{k-1}<(\rho_{1,k+1}^2 - \rho_{1,k+2}^2)/C_{k+1}$, which must be checked for affected indices after removal. This condition ensures that the resulting allocation admits a strictly increasing sequence of integer sample sizes and that each retained model contributes effectively to variance reduction.

This dynamic adjustment enables the active model subset to adapt to the available computational budget $p$, yielding allocations that maintain strict monotonicity in sample sizes while offering improved variance reduction compared to fixed-hierarchy approaches that permit identical allocations. The complete procedure is detailed in Algorithm~\ref{algo:Iterative_MFMC_Algo}.
