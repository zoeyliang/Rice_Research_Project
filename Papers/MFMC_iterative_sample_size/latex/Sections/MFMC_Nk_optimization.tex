...
\cite[Sec.~3.5]{BPeherstorfer_KWillcox_MDGunzburger_2016a}, even if the original set of models
does not satisfy \eqref{eq:Sample_size_real_assumptions_a} or \eqref{eq:Sample_size_real_assumptions_b},
one can select a subset of $K$ models that meet these requirements.
See Algorithm~1 in \cite{BPeherstorfer_KWillcox_MDGunzburger_2016a}.
...
%The analytical solution of \eqref{eq:Optimization_sample_size_m_relaxed} derived in Theorem~\ref{thm:Sample_size_real} is valid only when the underlying models satisfy the assumptions specified in \eqref{eq:Sample_size_real_assumptions}. Consequently, the practical implementation of MFMC proceeds in two stages. First, given a pool of candidate models, we identify a subset whose parameters satisfy the conditions of Theorem~\ref{thm:Sample_size_real}.  Among all admissible subsets, the chosen set should minimize the resulting variance 
%$\mathcal{V}^{\text{MF}}$ in \eqref{eq:MFMC_variance_optimal}, thereby ensuring maximal efficiency of the multifidelity estimator~\cite{PeWiGu:2016}. Once the subset is selected and the corresponding sample sizes are determined from the analytical expressions, the statistical sampling is carried out accordingly.
%
%It is important to emphasize that the model selection step is independent of the total computational budget~$p$. The optimal subset depends only on the correlation structure among models and their relative computational costs.
%
%
