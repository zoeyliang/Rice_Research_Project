...
\begin{equation}\label{eq:Iterative_integeb_sample_size_cost_bound}
...
\end{equation}
confirming that the iterative scheme never exceeds the prescribed computational budget. To establish the lower bound in \eqref{eq:Iterative_integeb_sample_size_cost_bound}, we compare the auxiliary sequences $M_k^*$ and $N_k^*$. From \eqref{eq:Tk_bound},
%
\[
...
\eqref{eq:Iterative_integeb_sample_size_cost_bound}.  
...
The upper bound in \eqref{eq:Iterative_integeb_sample_size_cost_bound} is attained if and only if no budget remains unused, i.e.,
\[
...
Theorem \ref{thm:MFMC_New_IntegerValued_Variance} establishes that the iterative integer-valued allocation achieves better variance performance compared to direct flooring in trade of the under-utilized budget due to floor operation.
...
%
\begin{equation}\label{eq:Iterative_Integeb_Variance_Bound}
...
\begin{proof}
We first prove the upper bound in \eqref{eq:Iterative_Integeb_Variance_Bound}.  
...
\]
which corresponds to the continuous optimal allocation $M_k^* = N_k^*$. Therefore, equality in \eqref{eq:Iterative_Integeb_Variance_Bound} holds if and only if the iterative scheme reproduces the continuous solution exactly, i.e., when all $\lfloor M_k^* \rfloor = N_k^*$ and the total cost equals $p$.
\end{proof}
