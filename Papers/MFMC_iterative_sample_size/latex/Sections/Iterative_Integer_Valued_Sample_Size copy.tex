...
% ====================================================
To address the budget under-utilization inherent in direct rounding strategies, as noted in \eqref{eq:bounds_for_floor}, we develop an iterative sample size estimation scheme for MFMC grounded in dynamic programming principles. The method applies Bellman’s principle of optimality~\cite{Be:1957} to decompose the global allocation problem into a sequence of subproblems. Each subproblem preserves the analytical structure of the continuous optimum while maintaining the variance–cost trade-off of the original formulation. The resulting recursive scheme yields an integer-compatible allocation strategy that achieves improved variance reduction without sacrificing budget feasibility.
...
Under the assumptions of Theorem~\ref{thm:Sample_size_real}, the iterative allocation defined by \eqref{eq:MFMC_New_RealValued_Sample_Size} satisfies 
\[
H_1^* < H_2^* < \cdots < H_K^*,
\]
forming a strictly increasing sequence.
\end{theorem}
%
\begin{proof}
Define the aggregate cost–variance partial sum from model $k$ to $K$
%
\begin{equation}\label{eq:aggregate_cost_variance_weight_Sk}
S_k = \sum_{j=k}^K \sqrt{C_j \Delta_j}.
\end{equation}
%
Then $S_{k-1} = \sqrt{C_{k-1}\Delta_{k-1}} + S_k$, and the iterative allocation satisfies
%
\begin{equation}\label{eq:Hk_definition}
H_k^* = \sqrt{\frac{\Delta_k}{C_k}} \frac{R_k}{S_k}, 
\qquad 
R_k = R_{k-1} - C_{k-1} H_{k-1}^* = 
% R_{k-1}\left(1 - \frac{\sqrt{C_{k-1}\Delta_{k-1}}}{S_{k-1}}\right)
R_{k-1} \frac{S_k}{S_{k-1}}.
\end{equation}
%
It follows that
%
\begin{equation}\label{eq:Rk_over_Sk}
\frac{R_k}{S_k} = \frac{R_{k-1}}{S_{k-1}}.
\end{equation}
%
Using assumption (ii) of Theorem~\ref{thm:Sample_size_real}, which ensures $\Delta_k/C_k$ is increasing with $k$, we obtain
%
\[
H_k^* = \sqrt{\frac{\Delta_k}{C_k}} \frac{R_k}{S_k} 
> \sqrt{\frac{\Delta_{k-1}}{C_{k-1}}} \frac{R_{k-1}}{S_{k-1}} = H_{k-1}^*.
\]
%
Hence, the sequence $\{H_k^*\}$ is strictly increasing.
\end{proof}
%
...
The following theorem establishes the fundamental equivalence between the iterative formulation and the standard MFMC sample allocation. This result guaranties that the sequential procedure exactly reproduces the optimal continuous allocation, thereby preserving both the total computational budget and the minimal achievable variance of the estimator.
...
\]
Moreover, the iterative scheme exactly satisfies the total budget constraint and yields the optimal variance,
\[
