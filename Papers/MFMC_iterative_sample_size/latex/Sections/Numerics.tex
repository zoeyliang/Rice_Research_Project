...
Table~\ref{Tab:Eg1_Parameters} summarizes the model parameters derived from a plasma physics application \cite{ElLiSa:2023,ElLiSa:2025}. The hierarchy consists of five fidelity levels with strong inter-model correlations ($\rho_{1,k} > 0.98$) and rapidly decreasing computational costs.
...
These results demonstrate that the proposed iterative allocation achieves superior resource utilization while minimizing the statistical efficiency loss imposed by integer constraints. The improvement is most significant in the practically relevant regime of moderate budgets, where computational resources are limited.
...
For a moderate budget of $b=200$, the real-valued allocation achieves an optimal variance of $f=5.0571\times10^{-5}$. Direct flooring results in substantial budget underutilization ($b \approx 179.34$) and a 15\% variance increase to $f=5.8074\times10^{-5}$, whereas the iterative scheme maintains exact budget adherence while reducing the variance penalty to just 6\% above the continuous optimum ($f=5.3495\times10^{-5}$). 
...
\caption{Comparison of real-valued and integer-valued MFMC sample allocations for small and large budgets ($b=46$ and $b=200$). The iterative integer-valued scheme achieves better cost utilization than the direct flooring approach while preserving the structure of the continuous optimizer.}
