...
$k$ is not uses and we set $z_k = 0$; otherwise $z_k = 1$.
...
\begin{subequations}\label{eq:Sequential_Optimization}
    ...
    \min \quad &\sigma_1^2  \sum_{k=j}^K \frac{ \rho_{1,k}^2 - \rho_{1,k+1}^2}{m_k},   \\
       \text{s.t.}\quad & \sum_{k=j}^K C_km_k \le b_j,       \\
                                & m_j \ge m_{j-1}^*,\quad  m_k \ge m_{k-1}, \quad k=j+1\ldots,K,\label{eq:Sequential_Optimization_obj_c}\\
...
solution, which we denote by $m_{j|j}^*, \ldots, m_{K|j}^*$.
   ...
   \item[i.] The unique solution of \eqref{eq:Sequential_Optimization} with $m_{j-1}^* = 0$ is
    ...
    and the resulting optimal objective function value is
     \begin{equation}\label{eq:MFMC_variance_optimal-j}
       ...
       \end{equation}
    \item[ii.] If $b_j = b - \sum_{k=1}^{j-1} C_k m_{k|k}^*$, then $m_{j|j}^*$ given by
                  \eqref{eq:Sequential_Optimization_mjj} satisfy $0 < m_{1|1}^* < \ldots < m_{K|K}^*$.
    \item[iii.] If $b_j = b - \sum_{k=1}^{j-1} C_k m_{k|k}^*$, then 
                  then $m_{j|j}^*$ given by \eqref{eq:Sequential_Optimization_mjj} and
                 $m_j^*$ given by \eqref{eq:MFMC_RealValued_Sample_Size} satisfy
                  $m_1^* = m_{1|1}^*,  \ldots, m_K^* = m_{K|K}^*$.
    \end{itemize}
\end{theorem}
\begin{proof}
 \begin{itemize}
...
\subsection{Recursive computation of integer-valued sample size}  
Although the continuous allocation yields an analytically optimal solution, it cannot be implemented directly because sample sizes must be integers. Solving the corresponding integer program exactly is impractical, especially as $K$ increases, since the discrete optimization lacks the separable structure of the continuous relaxation. Our objective is therefore to preserve the analytical form of the continuous solution while constructing feasible integer-valued allocations in a computationally efficient manner.

To achieve this, we develop an integer-valued extension of the recursive formulation. The idea is to propagate the continuous allocation structure forward through the hierarchy while enforcing integer feasibility at each step. This produces a sequential scheme in which rounding decisions are incorporated into the recursion rather than imposed post hoc, thereby maintaining the monotonicity and budget-consistency properties of the continuous solution. Specifically, we define
%
...
\end{equation}
%
The procedure begins with the continuous estimate $M_1^*$, from which the integer allocation $\lfloor M_1^*\rfloor$ is obtained. The corresponding cost $C_1\lfloor M_1^*\rfloor$ is subtracted from the total budget, and the recursion proceeds to subsequent levels using the remaining budget. This construction ensures that each integer allocation $\lfloor M_k^*\rfloor$ is adapted to the updated residual cost, thereby maintaining feasibility and preserving the optimal proportionality implied by the continuous solution. At each iteration, only the residual budget and a cumulative sum over the fidelity hierarchy are updated, both of which can be performed in constant time. Consequently, the per-step computational cost remains $\mathcal{O}(1)$, yielding an overall linear complexity $\mathcal{O}(K)$ with respect to the number of fidelity levels. Owing to this linear scaling and the recursive dependence structure, the scheme avoids global re-optimization or backtracking and is particularly well-suited for large or high-dimensional fidelity hierarchies, where efficiency and scalability are critical.


By design, the total cost of the integer allocation satisfies $\sum_{k=1}^K C_k\lfloor M_k^*\rfloor \le b$. To guarantee that each fidelity level receives at least one sample, the total budget must satisfy the lower bound
%
