...
% temp = delta./C;
% temp(2:end)-temp(1:end-1); %check second cond works for the selected model or not

%% Initialization
M=1;%2e5;
p_range = linspace(sum(C),p_max,M)';%sum(C) % the min budget >= sum(C)
        ...
        for k=2:length(N_iterative)
            ...
            if N_iterative(k) == N_iterative(k-1)
                ...
                if delta_k_1/C(k-1) < delta_k/C(k+1)
                    ...
                    restart_flag = true;
                    break; % break out of k-loop to restart while-loop            else
                end
            end
        end
    end % while restart_flag
...
%==========================================================================
figure(1);hold on;
h_max = max(max((f_floor-f_real_value)./f_real_value,(f_iterative-f_real_value)./f_real_value));
h_min = abs(min(min((f_floor-f_real_value)./f_real_value,(f_iterative-f_real_value)./f_real_value)));
loglog([sum(C),sum(C)],[1e-50,1e50], ':k','LineWidth',3,'MarkerSize',10);
loglog(p_range,f_diff_upper_bound./f_real_value,'o-','LineWidth',3,'color',[.7 0 0],'MarkerSize',5);
loglog(p_range,(f_floor-f_real_value)./f_real_value,'-','LineWidth',3,'color',[.2 0.4 1],'MarkerSize',30);
p2=loglog(p_range,(f_iterative-f_real_value)./f_real_value,'-.','LineWidth',3,'color',[.9 0.7 0],'MarkerSize',30);
p2.Color(4) = 0.6;
...
p2=loglog(p_range,(Cost_real_value-Cost_iterative)./p_range,'-.','LineWidth',3,'color',[.9 0.7 0],'MarkerSize',30);
p2.Color(4) = 0.6;
...
semilogy(Cost_real_value,f_real_value,'o-','LineWidth',3,'color',[.7 0 0],'MarkerSize',5);
semilogy(Cost_real_value,f_floor,'-.','LineWidth',3,'color',[.2 0.4 1],'MarkerSize',30);
semilogy(Cost_real_value,f_iterative,'-.','LineWidth',3,'color',[.9 0.7 0],'MarkerSize',30);

semilogy(Cost_floor,f_real_value,'-','LineWidth',3,'color',[.2 0.4 1],'MarkerSize',30);
semilogy(Cost_iterative,f_real_value,'-','LineWidth',3,'color',[.9 0.7 0],'MarkerSize',30);


ylim([min(f_real_value) max(f_real_value)*1.3])
xlim([sum(C)/1.09 p_max*1.1])
set(gca,'XScale', 'log','FontSize',20)

xlabel('Prescribed budget $p$','FontSize',25,'Interpreter','latex'); ylabel('f value','FontSize',25,'Interpreter','latex');
lgd = legend({'$p=\sum_{k}C_k$','Real-valued','Actual var -- direct floor','Actual var -- iterative','Actual cost -- direct floor','Actual cost -- iterative'},'Fontsize',14,'Interpreter','latex','Location','northeast');
lgd.FontSize = 17;
grid on
box on

% create a new pair of axes inside current figure
axes('position',[.59 .28 .25 .25])
hold on;
box on % put box around new pair of axes
indexOfInterest = (Cost_real_value < 5.31e3) & (Cost_real_value > 4.99e3); % range of t near perturbation
plot(Cost_real_value(indexOfInterest),f_real_value(indexOfInterest),'o-','LineWidth',3,'color',[.7 0 0],'MarkerSize',5); % plot on new axes
plot(Cost_real_value(indexOfInterest),f_floor(indexOfInterest),'-.','LineWidth',3,'color',[.2 0.4 1],'MarkerSize',30);
plot(Cost_real_value(indexOfInterest),f_iterative(indexOfInterest),'-.','LineWidth',3,'color',[.9 0.7 0],'MarkerSize',30);

plot(Cost_floor(indexOfInterest),f_real_value(indexOfInterest),'-','LineWidth',3,'color',[.2 0.4 1],'MarkerSize',30);
plot(Cost_iterative(indexOfInterest),f_real_value(indexOfInterest),'-','LineWidth',3,'color',[.9 0.7 0],'MarkerSize',30);
axis tight
xlim([5e3 5.31e3])
set(gca,'FontSize',13)
hold off;
...
%
% xlabel('Prescribed budget $p$','FontSize',25,'Interpreter','latex'); ylabel('f value','FontSize',25,'Interpreter','latex');
% lgd = legend({'$p=\sum_{k}C_k$','Real-valued','Direct floor','Iterative'},'Fontsize',14,'Interpreter','latex','Location','northeast');
% lgd.FontSize = 17;
% grid on
% box on
%
