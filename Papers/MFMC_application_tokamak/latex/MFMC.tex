...
\]
If $N_1,\ldots, N_K\in \mathbb{R}$ and $\alpha_2,\ldots,\alpha_K\in \mathbb{R}$ are feasible for 
\eqref{eq:Optimization_pb_sample_size0}, then $N_1,\ldots, N_K\in \mathbb{R}$ and 
$\alpha_2^*,\ldots,\alpha_K^*\in \mathbb{R}$ given by \eqref{eq:MFMC_SampleSize} are also feasible for
...
\begin{subequations}\label{eq:Optimization_pb_sample_size}
    \begin{align}
    \min  \; &\sum\limits_{k=1}^K C_kN_k,\\
        \label{eq:Optimization_pb_sample_size_var}
...
The variance constraint  \eqref{eq:Optimization_pb_sample_size_var} implies $N_1 > 0$.
Under suitable conditions, \eqref{eq:Optimization_pb_sample_size_var} has an analytical solution.
This is presented in the followingTheorem~\ref{thm:Sample_size_est}. 
The proof of Theorem~\ref{thm:Sample_size_est} is provided in the appendix.
