function ip = spcmpvalscbgpsp(d,outputLength,z,y,seq,fromindex,toindex,isgp)
	  ...
	  vals  = z(:,index:index+noldpoints-1);
		if isgp
		  nord  = n(1:oldndims);
			...
			ip(:,kstart:kend) = ...
  			ip(:,kstart:kend) + barypdstepgp(vals, outputLength, nord, order, ...
        getgpnodes(nord), y(kstart:kend,:), getgpbaryw(nord));
			...
			ip(:,kstart:kend) = ...
				ip(:,kstart:kend) + barypdstepcb(vals, outputLength,nord, order, ...
