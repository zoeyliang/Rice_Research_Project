function [ip, ipder] = spderivcb(d,outputLength,z,y,levelseq,purgedata)
...
for kl = 1:nlevels
	...
	if npoints > 1
		...
		if ndims > 1
			...
			valsnd = reshape(num2cell(vals', 2),np);
			s.type = '()';
			s.subs(1:ndims) = {':'};
		end
		l = uint16(1);
		while l <= ndims
			if ndims > 1
				...
				for l2 = 1:np(l)
					...
					tmpvals(l2,:) = num2cell(barypdstepcb(cell2mat(vals(:))',outputLength, tmpnord, tmporder, ...
