...
% V1.9.1  :July 23, 2020.   
%         Added patial support for vector output [Jiaxing Liang/Tonatiuh Sanchez-Vizuet]
...
enableDCT = spget(options, 'EnableDCT', 'off');
...
if isempty(zprev)
	nmaxprev = uint8(0);
	zmin = inf.*ones(outputLength,nresults);
	zmax = -inf.*ones(outputLength,nresults);
	xmin = zeros(outputLength,d,nresults);
	xmax = zeros(outputLength,d,nresults);
	fevalTime = 0;
	surplusCompTime = 0;
	npoints = uint32(0);
  levelseq = [];
	...
	for k = nmaxprev:nmax
		...
		[z(1:nresults,k+1),x{k+1}] = ...
				spevalf(gridgen, f, levelseq, d, [], range, varpos, ...
								vectorized,nresults,functionArgType,outputLength,val{:}); 
		...
		for l = 1:nresults
			[temp, id] = min(z{l,k+1},[],2);
            
            list = temp < zmin(:,l); 
			zmin(list,l) = temp(list);
			xmin(list,:,l) = x{k+1}(id(list),:);
				
			[temp, id] = max(z{l,k+1},[],2);
			list = temp > zmax(:,l);
			zmax(list,l) = temp(list);
			xmax(list,:,l) = x{k+1}(id(list),:);
...
% 			end
		...
		if k > 0
			...
			t0 = clock;
			for l = 1:nresults
				ip = zeros(outputLength,nsurpluses);
				for m = uint8(0):k-1
					oldlevelseq = spgetseq(m,d,options);
					ip = ip + feval(ipmethod, d, outputLength, z{l,m+1}, x{k+1}, ...
