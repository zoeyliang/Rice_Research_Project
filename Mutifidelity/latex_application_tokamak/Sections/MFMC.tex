...
Consider an ensemble of $K$ models $\{u_{h,k}\}_{k=1}^K$ ordered by decreasing fidelity, where each model $u_{h,k}$ is characterized by the standard deviation $\sigma_k$ of its output, the correlation coefficient $\rho_{1,k}$ with the highest-fidelity model $u_{h,1}$, and the computational cost per sample evaluation $C_k$. Define $\Delta_k = \rho_{1,k}^2 - \rho_{1,k+1}^2$ for $k = 1, \dots, K$, with the boundary convention $\rho_{1,K+1} = 0$. Assume the following conditions hold
%
\begin{alignat*}{3}
...
In practical implementation, the correlation coefficients $\rho_{1,k}$ and computational costs $C_k$ are typically unknown a priori and must be estimated via pilot sampling. Additionally, the theoretically optimal sample sizes $N_k^* \in \mathbb{R}$ require integer rounding for implementation. Departing from the conditional rounding strategy in \cite{GrGuJuWa:2023, PeWiGu:2016} (floor function when $N_k^* \ge 1$, ceiling otherwise), we adopt a uniform rounding scheme: all sample sizes are rounded up using the ceiling function $\lceil N_k^* \rceil$, with the feasibility condition $\sum_{k=1}^K C_k N_k^* \geq \sum_{k=1}^K C_k$ to exclude degenerate cases where $N_k^* < 1$ for all models. This approach guarantees $\mathbb{V}[A^{\mathrm{MF}}] \leq \epsilon_{\mathrm{tar}}^2$ since increased sample sizes reduce estimator variance in \eqref{eq:MFMC_variance_optimal}. The computational cost after rounding satisfies
%
...
\end{equation}
%
where the additive overhead $\sum_{k=1}^K C_k$ arises from the bound $N_k^*\le \lceil N_k^*\rceil< N_k^*+1$. The asymptotic behavior of this overhead is governed by sample size ordering: Condition (ii) of Theorem \ref{thm:Sample_size_est} implies strict monotonicity $N_1^* < \cdots < N_K^*$ in optimal sample sizes. When $\epsilon_{\mathrm{tar}}$ is sufficiently small to ensure $N_1^* \geq 1$, we obtain $\sum_{k=1}^K C_k \leq N_1^* \sum_{k=1}^K C_k \leq \sum_{k=1}^K C_k N_k^*$. This implies the additive rounding overhead is asymptotically dominated by the theoretical cost $\sum_{k=1}^K C_k N_k^*$. Consequently, the integer-rounded computational cost preserves the asymptotic scaling of the theoretical cost \eqref{eq:MFMC_sampling_cost}.
