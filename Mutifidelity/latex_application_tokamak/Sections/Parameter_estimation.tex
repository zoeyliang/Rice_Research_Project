...
\While{$\neg \text{converged}$}{
    ...
    \eIf{bivariate normality holds}{
        \If{$\max_k |t_k| \leq \delta$, $Q \geq \max\{30, Q_{\max \{idx\}} \}$ \text{as in } \eqref{eq:Pilot_sample_size_estimate} and $\text{idx}^{(Q)}=\text{idx}^{(p)}$ }{
            $\text{converged} \gets \text{True}$.
        }
    }{
        \If{$\max_k |t_k| \leq \delta$ and $\text{idx}^{(Q)}=\text{idx}^{(p)}$}{
            $\text{converged} \gets \text{True}$.
        }
    }
