...
%          Grau).
% V1.9   : July 29, 2020
%           Added partial support for vector outputs
%           [Jiaxing Liang/Tonatiuh Sanchez-Vizuet]
...
outputLength = size(z.fevalRange,1);
...
if length(varargin) == 1 && d > 1 
	...
	if ndims(y) == 2
		if size(y,1) == 1 || size(y,2) == 1
			...
			y = y(:)';
			orgsize = [outputLength, 1];
		else
			orgsize = [outputLength,size(y,1)];
		end
	else
...
if deriv, ipder = zeros(outputLength,ninterp,d); end
if continuous, ipder2 = zeros(outputLength,ninterp,d); end
...
if strcmpi(sparseIndices, 'off')
	for k = nfrom:n
		...
		if deriv
			if continuous
				...
				[iptemp, ipdertemp] = feval(ipmethod,d,outputLength,vals{output,k+1},y,levelseq,pd);
			end
			ip = ip + iptemp;
			ipder = ipder + ipdertemp; 
		else	
			ip = ip + feval(ipmethod,d,outputLength,vals{output,k+1},y,levelseq,pd);
		end
	end
else
	if isfield(z, 'purgeData')
	...
	if deriv
	  if continuous
  		[ip, ipder, ipder2] = feval(ipmethod,d,outputLength,vals{output},y,indices,pd,...
			                            z.maxLevel);
		else
			[ip, ipder] = feval(ipmethod,d,outputLength,vals{output},y,indices,pd);
		end
	else
		ip = feval(ipmethod,d,outputLength,vals{output},y,indices,pd);
	end
end
