function [ip, ipder] = spderivcbsp(d,outputLength,z,y,seq,purgedata)
...
ipder = zeros(outputLength,ninterp,d);
...
while currentindex <= nlevels
  ...
  order = uint16(dimvec(order));
	...
	ip = ip + barypdstepcb(vals,outputLength, nord, order, getchebnodes(nord), y);
	
	% Compute derivatives
	if ndims > 1
		% make vals array ndims-dimensional
		valsnd = reshape(num2cell(vals', 2),npord);
		s.type = '()';
		s.subs(1:ndims) = {':'};
	end
	l = uint16(1);
	while l <= ndims
		if ndims > 1
			tmpvals = cell(npord(l),ninterp); 
            tmpvals(:) = {zeros(1,outputLength)};
			...
			for l2 = 1:npord(l)
				...
				vals = subsref(valsnd,s);
                    vals_tt = cell2mat(vals(:))';
					tmpvals(l2,:) = num2cell(barypdstepcb(vals_tt,outputLength, tmpnord, tmporder, ...
					                getchebnodes(tmpnord), y)',2);
			end
			s.subs{l} = ':';
		else
			tmpvals = num2cell(vals',2);
		end
		ipder(:,:,order(l)) = ipder(:,:,order(l)) + ...
			                    dctdiffcheb(nord(l), tmpvals,outputLength, y(:,order(l)));
		l = l + 1;
	end
