...
% V1.2  : July 29, 2020
%           Added partial support for vector output
...
if nargin < 3
	...
	if d > 1
		...
		for kl = 1:nlevels
            ...
            ztemp = num2cell(ztemp, 2); % ADD
			ztemp = reshape(ztemp, n);
			ztemp = permute(ztemp, order);
%             z(index:index+npoints-1) = ztemp(:);
			z(:,index:index+npoints-1) = cell2mat(reshape(ztemp,[],1))';%ztemp(:);
			index = index + npoints;
		end
	end
else
	...
	while currentindex <= toindex
		...
		if ndims > 1
			...
			ztemp = z(:,index:index+npoints-1)';
            ztemp = num2cell(ztemp, 2);
			ztemp = reshape(ztemp, n(1:ndims));
			ztemp = permute(ztemp, order);
            z(:,index:index+npoints-1) = cell2mat(reshape(ztemp,[],1))';
		end
		index = index + npoints;
