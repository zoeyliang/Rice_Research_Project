...
% ====================================================
The Monte Carlo finite element estimator is often computationally prohibitive due to the extensive sampling required on a fixed spatial discretization, particularly under stringent accuracy demands. To alleviate this cost, multilevel Monte Carlo (MLMC) methods \cite{Gi:2008,Gi:2015} have been widely adopted. In this work, we further explore the multi-fidelity Monte Carlo (MFMC) approach \cite{PeWiGu:2016}, which integrates surrogate models of varying fidelities without requiring explicit control over interpolation errors. MFMC applies the control variate technique to exploit statistical correlations between high- and low-fidelity models, yielding an efficient estimator that synthesizes information across model hierarchies.

The control variate approach  considers a  new random variable $u_{h,2}\left(\cdot, \boldsymbol{\omega}\right)$ such that it correlates with $u_{h}\left(\cdot, \boldsymbol{\omega}\right)$ but cheaper than $u_{h}\left(\cdot, \boldsymbol{\omega}\right)$ to evaluate, and whose expectation is either known or can be approximated with relatively small cost. Then we construct a new random variable
%
\[
...
\end{equation*}
%
where $u_{h,2}$ denotes a low-fidelity model and $\mu_2 = \mathbb{E}(u_{h,2})$ is its exact control variate mean and we can show that $\mathbb{E}(A^{\text{TF}}) = \mathbb{E}(u_h)$. The variance of this estimator is minimized by choosing $\alpha = -\frac{\text{Cov}(u_h, u_{h,2})}{\sigma_{u_{h,2}}^2}  = -\rho \sigma_{u_h}/\sigma_{u_{h,2}}$, where $\rho$ is the Pearson correlation coefficient between $u_h$ and $u_{h,2}$. In practice, this is achieved by evaluating both models using the same random samples $\boldsymbol{\omega}^{(i)}$, which maximizes correlation through sample reuse. With this choice, the variance becomes $\mathbb{V}(A^{\text{TF}}) = (1-\rho^2)\mathbb{V}(u_h)$. The approximate control variate Monte Carlo \cite{GoGeElJa:2020} handles the issue when the statistic of the lower fidelity model is unknown and the cost of the lower fidelity model is non trivial. The effectiveness of the control variate approach therefore depends on selecting a low-fidelity model that is strongly correlated with the high-fidelity model. It is clear that if $u_{h}$ and $u_{h,2}$ are highly correlated, then $\mathbb{V}(A^{\text{TF}})$ is much smaller than $\mathbb{V}(u_h)$ and MC simulation of $\mathbb{E}(A^{\text{TF}})$ converges much faster than that of $\mathbb{E}(u_h)$.
...
Extending this idea, the MFMC estimator incorporates the high-fidelity model $u_{h,1} = u_h$ along with a series of low-fidelity models. High-fidelity model offers accurate but computationally expensive representations of the underlying system, whereas low-fidelity models are less accurate but significantly cheaper to evaluate. The MFMC framework effectively balances this trade-off by allocating computational effort across fidelity levels. In doing so, it reduces the reliance on expensive high-fidelity evaluations while maintaining estimator accuracy and robustness. We now briefly review the core ideas underlying MFMC. 


Similar to the  two-fidelity model Monte Carlo, the multi-fidelity Monte Carlo combines the high-fidelity model $u_{h,1}=u_{h}$ with a sequence of progressively less accurate but computationally cheaper low-fidelity models $u_{h,k}: W \rightarrow Z$ for $k=2,\ldots,K$. Model fidelity decreases with increasing $k$, with $u_{h,1}$ representing the most accurate (high-fidelity) model. We define the variance and the Pearson correlation coefficient between any pair $u_{h,k}$ and $u_{h,j}$ as
%
\begin{equation*}
...
\end{equation*}
%
where $\text{Cov}(u_{h,k}, u_{h,j}) := \mathbb{E}[\langle u_{h,k} - \mathbb{E}(u_{h,k}), u_{h,j} - \mathbb{E}(u_{h,j})\rangle_Z]$ is the covariance and $\rho_{k,k}=1$ by definition.
...
% \end{equation}
The multi-fidelity Monte Carlo Finite-Element estimator $A^{\text{MF}}$ incorporates weighted corrections from the low-fidelity models into the  Monte Carlo estimator of the high-fidelity model, defined as
%
...
To estimate the correlation coefficients from a pilot sample of size $Q$, we use unbiased Monte Carlo estimators for the sample covariance and standard deviations of the high- and low-fidelity models. Let $\text{Cov}^{(Q)}$ denotes the empirical covariance between the high- and low-fidelity outputs, and $\sigma_1^{(Q)}$ and $\sigma_k^{(Q)}$ denote their respective sample standard deviations. The estimator for the correlation coefficient is given by
%
\[
