...
\end{equation}
%
Since $Y_k$ is the difference between two independent Monte Carlo estimators of the same model, it is unbiased: $\mathbb{E}(Y_k) = 0$ for $k\ge 2$. Consequently, the MFMC estimator is itself unbiased $\mathbb{E}(A^{\text{MF}}) = \mathbb{E}(u_{h,1})$. The variances of the correction terms $Y_k$ are
%
...
\end{equation}
%
Although $Y_k$ and $Y_j$ (for $2\le k<j \le K$) share overlapping sample sets and are therefore dependent, they are uncorrelated (see Lemma~\ref{lemma:Y_k_Y_j}). However, each $Y_k$ for $k\ge 2$ is correlated with $Y_1$. Using the covariance identity from \cite[Lemma~3.2]{PeWiGu:2016}, the covariances among these terms are 
%
\begin{equation}\label{eq:Cov_Yk}
\text{Cov}(Y_k,Y_j) =0,\quad \text{for } \;2\le k<j \le K,\qquad \text{Cov}(Y_1,Y_k) = - \left(\frac{1}{N_{k-1}} - \frac{1}{N_k}\right)\rho_{1,k}\sigma_1\sigma_k, \quad \text{for } \; k\ge 2.
\end{equation}
%
...
\]
%
where $C_k$ is the cost of generating a single sample of model $u_{h,k}$, and $N_k$ is the corresponding sample count. Unlike previous formulations \cite{PeWiGu:2016} that derive sample sizes based on a fixed computational budget, our approach directly expresses the sample sizes and computational resources in terms of the desired accuracy $\epsilon$. This formulation offers greater flexibility in applications where accuracy targets are more relevant than rigid cost constraints.

To determine the optimal sample sizes $N_k$ and weights $\alpha_k$,  we formulate an optimization problem that minimizes the total sampling cost $\mathcal{W}^{\text{MF}}$, subject to three constraints: first, the normalized statistical error $\mathcal{E}_{\text{Stat}}^2$ must meet the desired accuracy threshold $(1-\theta)\epsilon^2$; second,  the hierarchical ordering condition $N_{k-1}\le N_k$ for $k=2,\ldots, K$ ensures consistent sample reuse; and third, all sample sizes must be non-negative. The resulting constrained optimization problem is
%
...
\end{theorem}
%
The proof of Theorem \ref{thm:Sample_size_est} can be found in the Appendix. 
