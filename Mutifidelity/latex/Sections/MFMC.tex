...
offers an alternative by using the control variate technique to exploit correlations between a high-fidelity model and a set of lower-fidelity models. Unlike MLMC sampling with surrogates, MFMC accommodates diverse surrogate models without requiring explicit treatment of interpolation errors. Instead, it constructs an estimator that combines information from multiple fidelities in a computationally efficient manner, extending the utility and flexibility of MLMC to a broader range in application.


The Monte Carlo finite element estimator is often computationally expensive due to the extensive sampling on a fixed finite element discretization. This can become prohibitively costly, particularly under stringent accuracy requirements. To mitigate this challenge,
...
Previous analyses often assume prior knowledge of key parameters, such as correlation coefficients between high- and low-fidelity models and sample correction weights. However, in practice,  these parameters are typically unknown and must be estimated from sample statistics. The accuracy of these estimates directly affects the robustness of multifidelity Monte Carlo sampling, as errors in parameter estimation can propagate through the sampling process, reducing both computational efficiency and solution accuracy. Consequently, a systematic approach is required to determine an appropriate pilot sample size that ensures reliable parameter estimation. To this end, we examine the asymptotic behavior of correlation coefficient distributions and analyze the propagation of correlation estimation errors in the MFMC variance and cost-efficiency measures. By deriving sensitivity bounds on these quantities, we establish a stopping criterion for an adaptive, sequential sampling strategy. This strategy iteratively refines the sample size until the relative error in efficiency and variance satisfies a prescribed threshold, ensuring that correlation estimates remain sufficiently accurate while minimizing unnecessary computational costs. Through real-time statistical feedback, this dynamic approach enhances MFMC robustness and efficiency.


To estimate these parameters, we use a pilot sample of size $Q$ generated though Monte Carlo sampling. While the estimators $\text{Cov}_w^{(Q)}, \sigma_w^{(Q)},$ and $\widehat\sigma_w^{(Q)} $ are unbiased, the correlation estimator 
%
\[
\rho_{1,k}^{(Q)} = \frac{\text{Cov}_w^{(Q)}}{\sigma_w^{(Q)}\widehat\sigma_w^{(Q)}}
\]
%
can exhibit bias due to the nonlinearity of the ratio estimator. We thus address this by considering the mean square error between the true and sample mean of the correlation coefficient, and decomposing the bias-variance components as
%
\begin{equation}
\label{eq:MSE_rho}
...
\end{equation*}
%
Previous studies \cite{Fi:1915, Ha:2007, Ri:1932, So:1913} have evaluated the accuracy of correlation coefficient estimations under the assumption of bivariate normality, yielding parameters $a_1 = -(\rho_{1,k}-\rho_{1,k}^3)/2,\, a_2 = (1-\rho_{1,k}^2)^2 $. In such contexts, Fisher's z-transformation is typically employed to construct confidence intervals for $\rho$ based on standard normal theory. In our study, the exact distribution of sample realizations remains unspecified, indicating potential nonnormality. Consequently, the asymptotic properties of sample statistics \cite{Og:2006,Pi:1937} is considered and generating $a_1 = 0,\, a_2 = 1 $. This approach presumes a sufficiently large sample size $Q$. However, our objective is to use a smaller $Q$. To achieve this, we consider sequential analysis \cite{Wa:1947}, which dynamically adjusts the sample size during the trial. This method permits early termination of the trial compared to traditional large fixed sample size upon meeting predefined stopping criteria, thereby conserving time and resources. 

To establish appropriate stopping criteria, we will first construct confidence intervals that account for potential nonnormality. Next, we will assess the sensitivity of cost efficiency and variance relative to the correlation coefficient. For confidence interval construction, we use the Spearman rank-order transformation\cite{BiHi:2017, FiHaPe:1957}, using the standard error as defined by Spearman$_F$ \cite{FiHaPe:1957}. In cases where the sample size is below 20, bootstrap methods \cite{Ef:1979,EfTi:1993} may also be considered. The Fisher $z^\prime$ transformation for $\rho_{1,k}$ is deifned as 
\[
z^\prime  = 0.5\ln \left(\frac{1+\rho_{1,k}}{1-\rho_{1,k}}\right),
\]
then the 95\% confidence interval for $z^\prime$ is $z^\prime \pm 1.96\sigma_{z^\prime}$, where the estimate of the standard error is $\sigma_{z^\prime} = 1.03/\sqrt{Q-3}$. Taking the upper and lower bounds of the confidence interval for ${z^\prime}$, we can revert back to the confidence interval for $\rho_{1,k}$ as 
%
\begin{equation}
\label{eq:Confidence_Interval_rho}
    \rho_{1,k} = \frac{e^{2z^\prime}-1}{e^{2z^\prime}+1}.
\end{equation}
%

Next, we examine the sensitivity of cost efficiency and variance concerning the correlation coefficient. Let  $\boldsymbol{\rho}$ represent the true correlation coefficient vector, approximated by $\boldsymbol{\rho}+\Delta \boldsymbol{\rho}$. Applying a first-order Taylor expansion, the propagated error in cost efficiency and variance, resulting from the estimation of the correlation coefficient, can be approximated as
...
%
%
where the sensitivity of $\xi$ and $\mathbb{V}\left(A^{\text{MF}}\right)$ to correlation perturbations are given by
%
\begin{align*}
...
\text{with} \quad S& = \sum_{k=1}^K\sqrt{C_k\left(\rho_{1,k}^2-\rho_{1,k+1}^2\right)},\quad
...
\end{align*}
%
Note since $S^\prime$ is always negative under MFMC conditions, both $\partial \xi/\partial \rho_{1,k}$ and $\partial \mathbb{V}\left(A^{\text{MF}}\right)/\partial \rho_{1,k}$ are negative. This means as correlation coefficient $\rho_{1,k}$ increases, both efficiency will improve and helps reduce statistical errors. Moreover, 
The larger the magnitude of $\partial \xi/\partial \rho_{1,k}$ and $\partial \mathbb{V}\left(A^{\text{MF}}\right)/\partial \rho_{1,k}$, the more sensitive of $\xi$ and $\mathbb{V}\left(A^{\text{MF}}\right)$ with respect to $\rho_{1,k}$.  Applying the Cauchy-Schwarz inequality, the error bound follows as
%
...
\end{align}
%
We now derive the relative error bounds on efficiency and variance of MFMC, Given an accuracy $\delta$, we find the maximum of $A_0$ and $A_1$ as $A$, so each correlation estimation error satisfies $|\Delta \rho_{1,k}| \leq \delta/(A\sqrt{K-1})$. this will be used as the stopping criterion for the dynamic algorithm for selecting an optimal sample size $Q$. We propose a dynamic strategy, also known as sequential analysis \cite{La:2001,Wa:1947}, that iteratively updates sample statistics via Welford’s method while adaptively increasing sample sizes until the stopping criterion is met, as outlined in Algorithm \ref{algo:Parameter_Estimation}. This process incorporates the backtracking model selection procedure in Algorithm \ref{algo:enhanced_mfmc_selection} in Section \ref{sec:Model_Selection}.
...
\DontPrintSemicolon
    ...
    Using \eqref{eq:delta_xi_bound} and \eqref{eq:delta_var_bound} to compute threshold for $\boldsymbol{\rho}$.
    ...
    \If{$\left|\frac{\xi^{(p+dQ)}-\xi^{(p)}}{\xi^{(p+dQ)}}\right|<\delta$ and $\frac{\left|\mathbb{V}\left(A^{\text{MF}}\right)^{(p+dQ)}-\mathbb{V}\left(A^{\text{MF}}\right)^{(p)}\right|}{\mathbb{V}\left(A^{\text{MF}}\right)^{(p+dQ)}}<\delta$}
    ...
    {
    \If{ the length of confidence interval is bigger than $2|\Delta \rho_{1,k}|$}
    {
    \text{AddSample = False}
    }
    \Else {
