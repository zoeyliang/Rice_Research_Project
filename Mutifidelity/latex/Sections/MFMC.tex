...
The Welford algorithm provides unbiased estimators for the mean, variance, and covariance between high- and low-fidelity models, satisfying $\mathbb{E}(m_w^{(N)})=\mu_1$ and $\mathbb{E}(\widehat m_w^{(N)})=\mu_k$. The proxy for variance follows $\mathbb{E}(v_w^{(N)})=(N-1)\sigma_1^2$, the standard deviations are unbiased and follow $\mathbb{E}(\sigma_w^{(N)})=\sqrt{\mathbb{E}(v_w^{(N)})/(N-1)}=\sigma_1$ and $\mathbb{E}(\widehat \sigma_w^{(N)})=\sigma_k$. Similarly, the expectation of the covariance is unbiased, following $\mathbb{E}( r_w^{(N)})=(N-1)\rho_{1,k}\sigma_1\sigma_k$ and $\mathbb{E}(\text{Cov}_w^{(N)}) = \mathbb{E}( r_w^{(N)})/(N-1) = \rho_{1,k}\sigma_1\sigma_k$. 

Even though $\text{Cov}_w^{(N)}, \sigma_w^{(N)}, \widehat\sigma_w^{(N)} $ are all unbiased, their ratio $\rho_{1,k}^{(N)} = \frac{\text{Cov}_w^{(N)}}{\sigma_w^{(N)}\widehat\sigma_w^{(N)}}$ may not be unbiased. Treating this sample correlation coefficient as our estimator for $\rho_{1,k}$. To analyze its properties, consider the parameter vector $q=(\rho_{1,k}\sigma_1\sigma_k,\sigma_1,\sigma_k)^T, q_N=(\text{Cov}_w^{(N)},\sigma_w^{(N)},\widehat\sigma_w^{(N)})^T$. A second-order Taylor expansion of
$\rho_{1,k} =f(q) = q_1/(q_2 q_3)$ about $q$ reveals
%
\begin{equation}
\label{eq:Correlated_Coeff_approx}
  \rho_{1,k}^{(N)} \approx \rho_{1,k} + \nabla f |_{q} \cdot (q_N-q) + \frac 1 2 (q_N-q)^T
    \begin{bmatrix}
        0&-\frac{1}{\sigma_1^2 \sigma_k}&-\frac{1}{\sigma_1\sigma_k^2}\\
        -\frac{1}{\sigma_1^2 \sigma_k}&\frac{2\rho_{1,k}}{\sigma_1^2}&\frac{\rho_{1,k}}{\sigma_1\sigma_k}\\
        -\frac{1}{\sigma_1\sigma_k^2}&\frac{\rho_{1,k}}{\sigma_1\sigma_k}&\frac{2\rho_{1,k}}{\sigma_k^2}\\
    \end{bmatrix}
    (q_N-q),  
\end{equation}
%
where $\nabla f|_{q} = (\frac{1}{\sigma_1\sigma_k},-\frac{\rho_{1,k}}{\sigma_1},-\frac{\rho_{1,k}}{\sigma_k} )^T$. Since $\text{Cov}_w^{(N)},\sigma_w^{(N)},\widehat\sigma_w^{(N)}$ are unbiased, the expectation of the first-order term in the Taylor expansion is zero. By the central limit theorem, The sample covariance estimator $\text{Cov}_w^{(N)}$ is asymptotically normal and follows $\sqrt{N}(\text{Cov}_w^{(N)}-\rho_{1,k}\sigma_1\sigma_k)\sim \mathcal{N}(0,\sigma_1^2\sigma_k^2(1+\rho_{1,k}^2))$, we obtain $\text{Cov}_w^{(N)}-\rho_{1,k}\sigma_1\sigma_k=\mathcal{O}(1/\sqrt{N})$. Similarly, we obtain each entry in $q_N-q$ is $\mathcal{O}(1/\sqrt{N})$.
...
\Sigma =
\begin{bmatrix}
\text{Var}(q_1)&\text{Cov}(q_1,q_2)&\text{Cov}(q_1,q_3)\\
\text{Cov}(q_2,q_1) &\text{Var}(q_2)&\text{Cov}(q_2,q_3)\\
\text{Cov}(q_3,q_1)&\text{Cov}(q_3,q_2)&\text{Var}(q_3)
\end{bmatrix}
\]
%
To compute $\text{Cov}(q_1,q_2) = \mathbb{E}(q_1q_2)-\mathbb{E}(q_1)\mathbb{E}(q_2)$, since both $\mathbb{E}(q_1) = \rho_{1,k}\sigma_1\sigma_k$ and $\mathbb{E}(q_2)=\sigma_1$ are unbiased, then 
$\text{Cov}(q_1,q_2) = \mathbb{E}[(q_1-\rho_{1,k}\sigma_1^2\sigma_k)\cdot (q_2-\sigma_1)]$. Let $s^2 = 1/(N-1)\sum_{i=1}^N$

$\text{Cov}(q_1,q_2) = \mathbb{E}[(q_1-\rho_{1,k}\sigma_1^2\sigma_k)\cdot (s^2-\sigma_1^2)]$


$q_1=\rho_{1,k}\sigma_1\sigma_k + \mathcal{O}(1/\sqrt{N})$, $q_2=\sigma_1 + \mathcal{O}(1/\sqrt{N})$, then $\mathbb{E}(q_1q_2) = \rho_{1,k}\sigma_1^2\sigma_k + \mathcal{O}(1/N)$. This implies that $\text{Cov}(q_1,q_2)\approx$
\[
\Sigma\approx \frac{1}{N}
\begin{bmatrix}
\sigma_1^2\sigma_k^2(1+\rho_{1,k}^2) &\rho_{1,k}\sigma_1^3\sigma_k&\rho_{1,k}\sigma_1\sigma_k^3\\
\rho_{1,k}\sigma_1^3\sigma_k &\frac{\sigma_1^2}{2} &0\\
\rho_{1,k}\sigma_1\sigma_k^3&0&\frac{\sigma_k^2}{2}
\end{bmatrix}
\]
The expectation of the second order term in the Taylor expansion
satisfies $\mathbb{E}((q_N-q)^TH(q_N-q)) = \text{tr}(H\Sigma) + \mu^TH\mu$ with $\mu=\mathbb{E}(q_N-q)=0$ and $\Sigma = \text{Cov}(q_N-q)$, and $H$ is the Hessian in \eqref{eq:Correlated_Coeff_approx}.
%
\begin{equation}
\label{eq:Expectation_rho}
    \mathbb{E}\left(\rho_{1,k}^{(N)}\right) \approx \rho_{1,k}+\frac{1}{2}\text{tr}(H\Sigma)=\rho_{1,k}+\frac{\rho_{1,k}(1-\sigma_1-\sigma_k)}{N} = \rho_{1,k} + \mathcal{O}\left(\frac 1 N\right).
\end{equation}
%
Therefore, 

By the central limit theorem,  $q_N$ converges asymptotically in probability to $q$ in distribution, following $\sqrt{N}(q_N-q)\sim \mathcal{N}(0,\Sigma)$ with the covariance matrix $\Sigma$, yielding the asymptotic variance of $\rho_{1,k}^{(N)}$ via the multivariable delta method
%
\begin{equation}
    \label{eq:Var_rho}
...
\end{equation}
%
The total estimation error decomposes into bias and variance components
%
\begin{equation*}
...
\end{equation*}
%
To bound the relative error by $\delta_1$, we allocate a fraction $\theta_1$ to bias and $1-\theta_1$ to variance. For the bias term, using \eqref{eq:Expectation_rho}, we have $N\ge \frac{C_\rho}{\theta_1\delta_1}$. For the variance term, consider Chebyshev inequality
%
\[
...
% \]
Let $\boldsymbol{\rho}$ be the true correlation vector and approximating it by $\boldsymbol{\rho}+\Delta \boldsymbol{\rho}$. Error propagation to the cost efficiency $\xi$ is assessed via
%
\[
...
\end{align*}
%
Note The term $\left(\frac{C_k\rho_{1,k}}{\sqrt{C_k(\rho_{1,k}^2-\rho_{1,k+1}^2)}}-\frac{C_{k-1}\rho_{1,k}}{\sqrt{C_{k-1}(\rho_{1,k-1}^2-\rho_{1,k}^2)}}\right)$ in $\partial \xi/\partial \rho_{1,k}$ is always negative due to the conditions required for MFMC. Applying Cauchy-Schwarz, we bound the relative error
%
\[
\left|\frac{\Delta \xi}{\xi}\right|\le \underbrace{\frac{1}{\xi}\sqrt{\sum_{k=2}^K \left(\frac{\partial \xi}{\partial \rho_{1,k}}\right)^2}}_{C^\prime} \cdot \sqrt{\sum_{k=2}^K\left(\Delta\rho_{1,k}\right)^2}\le C^\prime \delta_2<\delta
\]
%
