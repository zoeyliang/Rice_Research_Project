...
\end{align}
%
Using the variance of the MFMC estimator, the nMSE for the multi-fidelity Monte Carlo estimator can be decomposed into contributions from bias and statistical errors
%
\[
...
\]
%
To achieve a balance between these two components, the splitting ratio $\theta$ is introduced, as previously established in the Monte Carlo Finite-Element framework. This ratio ensures that the discretization error remains within the bound $\theta \epsilon^2$. The spatial resolution required to achieve this accuracy can be determined by estimating the number of spatial grid points $M_L$ and the corresponding grid level $L$, given by
%
\begin{equation}
...
\]
%
Our next goal is to determine the optimal sample sizes $N_k$ and weights $\alpha_k$ for the MFMC estimator \eqref{eq:MFMC_estimator_independent}. Unlike previous approaches, such as those presented in \cite{PeWiGu:2016}, which derive sample sizes based on a fixed computational budget, our reformulation focuses on a user-defined accuracy parameter $\epsilon$. While both methodologies yield equivalent results, our approach emphasizes adaptability to accuracy requirements by explicitly linking the desired precision to the sample sizes and computational resources. This perspective provides a flexible and practical framework, particularly suited for scenarios where accuracy requirements are prioritized over pre-defined cost constraints. To achieve this,  we formulate an optimization problem that minimizes the total sampling cost $\mathcal{W}^{\text{MF}}$, subject to three key constraints. First, the normalized statistical error $\mathcal{E}_{\text{Stat}}^2$ must meet the desired accuracy threshold $(1-\theta)\epsilon^2$. Second,  the hierarchical ordering condition $N_{k-1}\le N_k$ for $k=2,\ldots, K$ ensures a logical allocation of samples across fidelity levels. Third, all sample sizes must be non-negative. The resulting optimization problem is
%
