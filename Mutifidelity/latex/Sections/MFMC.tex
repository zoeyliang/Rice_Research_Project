...
To maximize the capability of the multi-fidelity Monte Carlo estimator relative to the Monte Carlo estimator, we aim to select a subset of models from the available set such that the cost efficiency ratio $\xi$ (or equivalently, the sampling cost $\mathcal{W}^\text{MF}$) is minimized. Let $\mathcal{S}=\{u_{h, k}\}_{k=1}^K$ denote the $K$ available models. Our goal is to identify a subset  $\mathcal{S}^*=\{u_{h, i}\}_{i=1}^{K^*} \subseteq S ( \text{ with } K^*\le K)$ that minimizes $\xi$  subject to the conditions of Theorem \ref{thm:Sample_size_est} -- noting that $\mathcal{S}^*$ is non-empty since the high-fidelity model $u_{h,1}$ is always included. Unlike the exhaustive algorithm in \cite[Algorithm~1]{PeWiGu:2016}, which incurs a computational cost of $\mathcal{O}(2^K)$, our method uses a backtracking strategy that incrementally selected the subsets and prunes branches that cannot yield valid complete solutions. This approach significantly reduces search space and lowers the computational cost to $\mathcal{O}(K^2)$, avoiding a large number of unnecessary computations for large $K$. The resulting algorithm, presented in Algorithm \ref{algo:enhanced_mfmc_selection}, returns the selected $K^*$ low-fidelity models along with their corresponding correlation coefficients $\boldsymbol{\rho}$, costs $\boldsymbol{C}$, the minimal cost efficiency ratio $\epsilon_{\text{min}}$, and weights $\alpha_i$ for $i=2\ldots, K^*$. 

\begin{equation*}\label{eq:Optimization_pb_model_selection}
    \begin{array}{lll}
    \displaystyle\min_{\{i_1,i_2, \ldots,i_{K^*}\}\in S} &\displaystyle \xi,\\
...
\DontPrintSemicolon    
   \KwIn{$K$ candidate models $u_{h, k}$ with coefficients $\rho_{1,k}$, $\sigma_1$, $\sigma_k$ and cost per sample $C_k$.}\vspace{1ex}
    
    \KwOut{Selected $K^*$ models $u_{h, i}$ in $\mathcal{S}^*$, with coefficients $\rho_{1,i}$, $\alpha_i$ and $C_i$ for each model $u_{h, i}$.}\vspace{1ex}
    \hrule \vspace{1ex}
...
\Input{%
$K$ candidate models $\{u_{h,k}\}_{k=1}^K$ with correlation coefficients $\rho_{1,k}$, standard deviations $\sigma_1$ and $\sigma_k$, and costs per sample $C_k$. 
}
\Output{%
  Optimal subset of models $\mathcal{S}^*=\{u_{h,i}\}_{i=1}^{K^*} \subseteq \{u_{h,k}\}_{k=1}^K$, correlations of selected models $\boldsymbol{\rho}^*$, costs of selected models $\boldsymbol{C}^*$, minimal cost efficiency ratio $\xi_{\text{min}}$, weights $\alpha_i$ for $i=2,\ldots, K^*$.
}
\hrule


Sort models by descending $|\rho_{1,k}|$, relabel $\rho_{1,k}, C_k$ for $k=1,\ldots,K$ and $\{u_{h,k}\}_{k=1}^K$.

Add dummy model $u_{h,K+1}$ with $\rho_{1,K+1} = 0$. 

Initialize global variables: 
$\mathcal{S} = \{u_{h,1}\}$, $\boldsymbol{\rho}=[1]$, $\boldsymbol{C}=[C_1]$, $\xi=0$, $\xi_{\text{min}}=1$.

\vspace{3mm}

\Fn{ $[\mathcal{S}^*,\, \boldsymbol{\rho}^*, \,\boldsymbol{C}^*]$ = \textbf{Backtrack}$\left(\mathcal{S}, \,\boldsymbol{\rho}, \,\boldsymbol{C}, \,k_{\text{last}}\right)$}{

Compute $\xi$ via \eqref{eq:MFMC_sampling_cost_efficiency}. 


  \If{$\xi\geq \xi_{\text{min}}\,$ or $\,\xi>1$ }{
    \Return 
  }
  \Else {
    $\mathcal{S}^* = \mathcal{S}$, \\
    $\boldsymbol{\rho}^* = \boldsymbol{\rho}$, \\
  ...
  \For{$k = k_{\text{last}}$ \textbf{to} $K$}{ 
    ...
    \If{%
      $|\boldsymbol{\rho}_{\text{end}}|>|\rho_{1,k}|$ \textbf{and} 
      $\frac{\boldsymbol{C}_{\text{end}}}{C_k} > \frac{\boldsymbol{\rho}_{\text{end}}^2 - \rho_{1,k}^2}{\rho_{1,k}^2 - \rho_{1,k+1}^2}$ 
    }{
      
      \textbf{Backtrack} $(\mathcal{S} \cup \{u_{h,k}\},[\boldsymbol{\rho}, \rho_{1,k}], [\boldsymbol{C},C_k], k+1)$.
    }
  }
}
\vspace{3mm}

$[\mathcal{S}^*,\, \boldsymbol{\rho}^*, \,\boldsymbol{C}^*]=$ \textbf{Backtrack} $(\mathcal{S},\, \boldsymbol{\rho},\, \boldsymbol{C},\, 2)$. 

For $\mathcal{S}^*=\{u_{h,i}\}_{i=1}^{K^*}$, compute weights $\alpha_i$ for $i=2,...,K^*$ via \eqref{eq:MFMC_coefficients}. 
