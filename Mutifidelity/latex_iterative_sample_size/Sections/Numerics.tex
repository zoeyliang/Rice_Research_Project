...
\end{tabular}
\caption{
Budget sensitivity analysis for the plasma physics example. \textbf{Left:} Cost deviation $p - \sum_k C_k \lfloor \cdot \rfloor$ versus prescribed budget $p$. \textbf{Right:} Variance deviation $\sum_k (\Delta_k/\lfloor \cdot \rfloor - \Delta_k/N_k^*)$ versus prescribed budget $p$. 
} 
\label{fig:Eg1} 
\end{figure}
%

Figure~\ref{fig:Eg1} examines the sensitivity of cost utilization and variance performance to the prescribed budget $p \in [\sum_k C_k, 10^6]$, illustrating the relative performance of integer allocation schemes compared to the continuous optimum.
